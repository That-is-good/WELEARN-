...
// @version      1.1.4
// @author       SSmJaE
// @description  显示WE Learn随行课堂题目答案；支持班级测试；自动答题；刷时长；基于生成式AI(ChatGPT)的答案生成，部分功能由櫻羽若俳改进
// @license      GPL-3.0
...
具体信息 : ${errorDetail.message}` : void 0;
  ...
  async function solveRecordingTasks() {
      const selectors = ['et-recorder', 'et-follow-me', 'et-talk'];
      const doc = document.querySelector('iframe') ? document.querySelector('iframe').contentDocument : document;
      const win = doc.defaultView || window;
      const angular = win.angular;
      ...
      try {
          ...
          rootScope = angular.element(appRoot).injector().get('$rootScope');
      } catch (e) {
          console.error("无法获取 Angular rootScope");
          return;
      }
      ...
      for (const tag of selectors) {
          ...
          for (const el of elements) {
              ...
              if (tag === 'et-recorder') {
                  // 普通录音：次数设为1，给一个虚假的文件名
                  dummyData = {
                      record_count: 1,
                      learner_response: "dummy_record_" + Date.now() + ".mp3",
                      result: "neutral"
                  };
              } 
              else if (tag === 'et-follow-me') {
                  ...
                  const dummyFiles = Array(sentences.length).fill("follow_me_dummy.mp3");
                  const dummyScores = Array(sentences.length).fill("100"); // 给100分

                  dummyData = {
                      record_count: 1,
                      learner_response: dummyFiles.join("[,]"),
                      result: dummyScores.join("[,]")
                  };
              }
              else if (tag === 'et-talk') {
                  // 对话：需要角色信息和文件名列表
                  const flows = el.querySelectorAll('flow[record]');
                  const dummyFiles = Array(flows.length).fill("talk_dummy.mp3");
                  dummyFiles.unshift("ROLE_0"); // 默认选择第一个角色

                  dummyData = {
                      record_count: 1,
                      learner_response: dummyFiles.join("[,]"),
                      result: Array(flows.length).fill("100").join("[,]")
                  };
              }

              // 【关键】利用 main.js 中的 answerRestore 信号
              // 平台组件监听到这个信号后会同步内部模型状态，认为“已答题”
              rootScope.$broadcast("answerRestore." + id, dummyData);
          }
      }
  ...
  async function solveEt(answers) {
    ...
    const etwordpracticeBtn = document.querySelector('et-button[action="wordbank.practice()"] button');
    const etrecordingTasks = document.querySelectorAll("et-recorder, et-follow-me, et-talk");
    const etmarkTasks = document.querySelectorAll("et-mark");
    if (etwordpracticeBtn) {
        etwordpracticeBtn.click();
        await sleep(1000);
        await solveWordPractice();
    }else if (etrecordingTasks.length > 0) {
        await solveRecordingTasksPersistent();
    }else if (etmarkTasks.length > 0) {
        await solveMarkTasks();
    }else {
        logger.debug("未检测到特殊题型，开始常规作答");
    }
    let liOrder = 0;
    let spanOrder = 0;
    let optionOrder = 0;
    for (const answer of answers) {
      ...
      switch (answer.type) {
        ...
        case "et-blank":{
            await (async () => {
                const innerSpan = blankOnPaper[blankOrder];
                if (!innerSpan) return;

                const win = innerSpan.ownerDocument.defaultView || window;
                const angular = win.angular;
                const targetText = answer.text.trim();

                // 1. 获取 et-blank 容器及其真实 ID (例如 u08e2a994...)
                const etBlankEl = innerSpan.closest('et-blank');
                if (!etBlankEl || !etBlankEl.id) {
                    innerSpan.textContent = targetText; // 没找到 ID 则执行普通填充
                    return;
                }
                const realId = etBlankEl.id;

                // 2. 核心：通过 Injector 获取 $rootScope (无视 scope 为 undefined 的限制)
                try {
                    // 在 Angular 生产模式下，只能通过应用根节点（通常是 body 或 .app）获取注入器
                    const appRoot = win.document.querySelector('.app, [ng-app], body');
                    const injector = angular.element(appRoot).injector();
                    const rootScope = injector.get('$rootScope');

                    if (rootScope) {
                        // 3. 核心真填入逻辑：
                        // 在 main.js 第 927 行，填空组件通过 t.$on("optionIn." + u, ...) 接收数据
                        // 派发此信号会直接修改填空框内部的 m.value，并触发第 935 行的进度更新 watch
                        rootScope.$broadcast("optionIn." + realId, targetText);
                        
                        // 4. 强制脏检查，确保进度 Measure 更新到提交逻辑 (g函数) 所需的变量中
                        if (!rootScope.$$phase) rootScope.$apply();
                        
                        // 5. 模拟物理点击，确保 UI 状态和焦点正确
                        const rect = innerSpan.getBoundingClientRect();
                        const clickEvt = {
                            bubbles: true, cancelable: true, view: win,
                            clientX: rect.left + rect.width / 2, clientY: rect.top + rect.height / 2
                        };
                        innerSpan.dispatchEvent(new MouseEvent('mousedown', clickEvt));
                ...
                // 6. 自动关闭可能弹出的选项窗口 (Picker)
                setTimeout(() => {
                    const picker = innerSpan.ownerDocument.querySelector('div.optionsPicker.visible');
                    if (picker) {
                        const closeBtn = picker.querySelector('a.close');
        ...
        case "et-matching":{
          await sleep(1000);
          await (async () => {
            const keyStr = answer.element.getAttribute("key");
            if (!keyStr) {
                console.error("未找到答案 key");
                return;
            }

            const matchingElem = document.querySelector('et-matching');
            if (!matchingElem) {
                console.error("未找到 et-matching 元素");
                return;
            }

            const circlesA = matchingElem.querySelectorAll('circle[data-circle="A"]');
            const circlesB = matchingElem.querySelectorAll('circle[data-circle="B"]');
            if (circlesA.length === 0 || circlesB.length === 0) {
                console.error("未找到圆圈元素，可能尚未渲染完成");
                return;
            }
            ...
            for (const pair of pairs) {
              let [left, right] = pair.split('-').map(Number);
              left -= 1;
              right -= 1;
              if (left >= 0 && left < circlesA.length && right >= 0 && right < circlesB.length) {
                  // 获取原生 DOM 元素（确保不是 jQuery 对象）
                  const leftCircle = circlesA[left];
                  const rightCircle = circlesB[right];
                  
                  // 触发点击事件（使用 dispatchEvent，避免 view 参数错误）
                  const clickEvent = new MouseEvent('click', { bubbles: true, cancelable: true });
                  leftCircle.dispatchEvent(clickEvent);
                  await sleep(store.userSettings.solveInterval);
                  
                  rightCircle.dispatchEvent(clickEvent);
                  await sleep(store.userSettings.solveInterval);
              }
          }
  ...
  async function solveWordPractice() {
    const practiceContainer = document.querySelector('et-word-practice');
    if (!practiceContainer) return;

    // 1. 如果在选择模式首页，默认开启第一个模式（根据单词选释义）
    const opening = practiceContainer.querySelector('.opening.current');
    if (opening) {
        const startBtn = opening.querySelector('button');
        if (startBtn) {
            startBtn.click();
            await sleep(1500); // 等待第一题加载
        }
    }
    ...
    while (practiceContainer.classList.contains('visible')) {
        ...
        // 定位当前正在显示的题目
        const currentItem = practiceContainer.querySelector('li.current et-single-practice');
        if (currentItem) {
            const wrapper = currentItem.querySelector('.wrapper');
            // 如果这题还没做（没有 done 或 correct 类）
            if (wrapper && !wrapper.classList.contains('done')) {
                
                // --- 逻辑 A：选择题 (choose-exp / choose-word / audio-exp) ---
                // Welearn 会在 li 标签上通过 ng-class 标记正确答案。
                ...
                // --- 逻辑 B：填空/拼写题 (type-in) ---
                    ...
                    const keySpan = currentItem.querySelector('span.key');
                    const inputField = currentItem.querySelector('input');
                    if (keySpan && inputField) {
                        const answer = keySpan.textContent.trim();
                        inputField.value = answer;
                        
                        // 触发输入事件通知 Angular 更新模型
                        const win = currentItem.ownerDocument.defaultView || window;
                        const $ = win.jQuery || win.$;
                        if ($) {
                            $(inputField).val(answer).trigger('input').trigger('change').trigger('blur');
                        }
                        
                        // 模拟按下回车键提交拼写
                        const enterEvt = new KeyboardEvent('keydown', { bubbles: true, keyCode: 13 });
                        inputField.dispatchEvent(enterEvt);
                    }
                }
            }
        }

        // Welearn 的练习在答对后会有大约 1.5s - 2s 的动画延迟才会跳到下一题
        await sleep(2500);

        // 如果用户手动关闭了窗口，则退出循环
        if (!practiceContainer.classList.contains('visible')) break;
    }
}/*
  async function solveWordPractice() {
    const practiceItems = document.querySelectorAll('et-single-practice');
    if (practiceItems.length === 0) return;

    for (const item of practiceItems) {
        // 如果该题已经做过，跳过
        const wrapper = item.querySelector('.wrapper');
        if (wrapper && (wrapper.classList.contains('done') || wrapper.classList.contains('correct'))) continue;

        const win = item.ownerDocument.defaultView || window;
        const angular = win.angular;
        const el = angular.element(item);
        const scope = el.scope();

        if (scope && scope.singlePractice && scope.singlePractice.word) {
            const wordData = scope.singlePractice.word;
            const type = scope.singlePractice.type;

            if (type === 'type-in') {
                // --- 场景 1：单词拼写 ---
                const correctWord = wordData.name;
                scope.$apply(() => {
                    wordData.answer = correctWord;
                    // 模拟按下回车触发提交判断
                    scope.singlePractice.checkInput(); 
                });
            } else {
                // --- 场景 2：三选一/四选一选择题 ---
                const correctKey = wordData.key; // 获取正确选项的索引
                scope.$apply(() => {
                    // 调用组件内部的 select 方法完成真填入
                    scope.singlePractice.select(correctKey);
                });
            }
        }
        await sleep(800); // 预留平台动画和反馈时间
    }
  }*/
